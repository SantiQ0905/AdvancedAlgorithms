...
def part1(transmissions, mcodes):
    ...
    print("=" * 80)
    print("PART 1: MALICIOUS CODE DETECTION")
    print("=" * 80)
    
    for i, t in enumerate((t1, t2), 1):
        print(f"\nTransmission {i}:")
        for j, m in enumerate((m1, m2, m3), 1):
            pos = first_occurrence(t, m)
            if pos > 0:
                print(f"  Mcode{j}: Found at position {pos} (true {pos})")
            else:
...
def part2(transmissions):
    print("\n" + "=" * 80)
    print("PART 2: LONGEST PALINDROMIC SUBSTRING")
    print("=" * 80)
    
    for i, t in enumerate(transmissions, 1):
        a, b = longest_palindrome_bounds(t)
        length = b - a + 1 if b >= a else 0
        print(f"\nTransmission {i}:")
        print(f"  Position range: {a} {b}")
        print(f"  Palindrome length: {length} characters")
        if length > 0 and length <= 50:
            palindrome = t[a-1:b]
            print(f"  Content: {palindrome}")
...
def part3(transmissions):
    print("\n" + "=" * 80)
    print("PART 3: LONGEST COMMON SUBSTRING")
    print("=" * 80)
    
    t1, t2 = transmissions
    a, b = longest_common_substring_bounds_in_s1(t1, t2)
    length = b - a + 1 if b >= a else 0
    print(f"\nLongest common substring between Transmission 1 and 2:")
    print(f"  Position in Transmission 1: {a} {b}")
    print(f"  Substring length: {length} characters")
    if length > 0 and length <= 50:
        substring = t1[a-1:b]
        print(f"  Content: {substring}")
    print("\n" + "=" * 80)
